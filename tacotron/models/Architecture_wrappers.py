...
class VAECell(RNNCell):
	"""Variational AutoEncoder Cell
	...
	def __init__(self, convolutional_layers, lstm_layers, pool_size, D_size):
		"""Initialize encoder parameters
		Args:
			convolutional_layers: Encoder convolutional block class
			lstm_layer: encoder bidirectional lstm layer class
		...
		#linear projection
		self._D_mean = tf.layers.Dense(units=D_size, name='VAE_D_mean')
		self._D_var = tf.layers.Dense(units=D_size, name='VAE_D_var')
	...
	def __call__(self, inputs, batch_size):
		...
		conv_output = self._convolutions(inputs)
		#lstm layers
		lstm_output = self._lstm(conv_output)
		#mean pooling layer
		mean_pool_output=tf.layers.average_pooling1d(inputs=lstm_output, pool_size=self.pool_size, strides=1, padding='same')
		#mean and var layer
		D_mean = self._D_mean(mean_pool_output)
		D_var = self._D_var(mean_pool_output)

		# compute kl divergence (mu, var) ~ N(0,1)
		kl_div = -0.5 * (D_var + 1 - D_mean ** 2 - tf.exp(D_var))
		kl_div = tf.reduce_mean(kl_div) / batch_size

		D_output = self.reparameterize(D_mean, D_var)
		output=tf.reduce_mean(D_output, axis=1)
...
class TacotronDecoderCell(RNNCell):
	...
	* : This is typically taking a vanilla LSTM, wrapping it using tensorflow's attention wrapper,
	...
	def __init__(self, prenet, attention_mechanism, rnn_cell, speaker_embedding, language_embedding, residual_embedding, frame_projection, stop_projection):
		...
		self._residual_embedding = residual_embedding
		#self._language_embedding = language_embedding
	...
	def __call__(self, inputs, state):
		...
		LSTM_input = tf.concat([prenet_output, state.attention, self._speaker_embedding, self._language_embedding, self._residual_embedding], axis=-1)
