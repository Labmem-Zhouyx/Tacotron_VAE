...
class VAEConvolutions:
	def __init__(self, is_training, hparams, activation=tf.nn.relu, scope=None):
		...
		Args:
			...
			scope: Postnet scope.
		"""
		super(VAEConvolutions, self).__init__()
		self.is_training = is_training
		self.kernel_size = hparams.VAE_conv_kernel_size
		self.channels = hparams.VAE_conv_channels
		self.activation = activation
		self.scope = 'VAE_conv_layers' if scope is None else scope
		self.drop_rate = hparams.tacotron_dropout_rate
		self.VAE_conv_num_layers = hparams.VAE_conv_num_layers

	def __call__(self, inputs):
		with tf.variable_scope(self.scope):
			x = inputs
			for i in range(self.VAE_conv_num_layers):
				x = conv1d(x, self.kernel_size, self.channels, self.activation,
					self.is_training, self.drop_rate, 'VAE_conv_layer_{}_'.format(i + 1)+self.scope)
			return x


class VAERNN:
	#VAE a stack of 2 bidirectional LSTM layers
	def __init__(self, is_training, layers=2, size=256, zoneout=0.1, scope=None):
		super(VAERNN, self).__init__()
		self.is_training = is_training
		self.layers = layers
		self.size = size
		self.zoneout = zoneout
		self.scope = 'VAE_LSTM' if scope is None else scope

		#Create forward LSTM Cell
		self._fw_cell = [ZoneoutLSTMCell(size, is_training,
										   zoneout_factor_cell=zoneout,
										   zoneout_factor_output=zoneout,
										   name='VAE_LSTM_fw_{}'.format(i + 1)) for i in range(layers)]
		...
		self._bw_cell = [ZoneoutLSTMCell(size, is_training,
										   zoneout_factor_cell=zoneout,
										   zoneout_factor_output=zoneout,
										   name='VAE_LSTM_bw_{}'.format(i + 1)) for i in range(layers)]
...
class GradientReversal:
	...
	def call(self,x,s=1.0):
		...
		def reverse_grad(op,grad):
			grad_tmp=tf.clip_by_norm(-grad*s,0.5)
			return grad_tmp
		g=tf.get_default_graph()
		with g.gradient_override_map({"Identity":op_name}):
			y=tf.identity(x)
		return y
...
class Speaker_Classifier:
	"""
	Fully connected layers used as inputs for the softmax.
	"""
	def __init__(self,is_training,layer_size=256,speaker_size=64,scope=None):
		super(Speaker_Classifier).__init__()
		self.layer_size = layer_size
		self.is_training = is_training
		self.speaker_size=speaker_size
		self.scope = 'Speaker_Classifier' if scope is None else scope
		self.Gradient_layer = tf.layers.Dense(units=self.layer_size, activation=tf.nn.relu, name='Gradient_layer')
		self.Rev_gradient = GradientReversal()
		self.Hidden_layer=tf.layers.Dense(units=self.layer_size,activation=tf.nn.relu,name='Hidden')
		self.Classifier_layer=tf.layers.Dense(units=self.speaker_size,activation=tf.nn.relu,name='Classifier')


	def __call__(self, inputs, grad_rev_scale):
		with tf.variable_scope(self.scope):
			#G = self.Gradient_layer(inputs)
			#H_in = self.Rev_gradient(G, grad_rev_scale)
			H_in = self.Rev_gradient(inputs, grad_rev_scale)
			H_out = self.Hidden_layer(H_in)
			outputs=self.Classifier_layer(H_out)
			return outputs
